...
## Order of work — fixed
1. **Ladder first.** For your topic run `node "${CLAUDE_PLUGIN_ROOT}/scripts/latest-search.mjs" "<topic>"` and read the dated rows it prints. It works with no setup (keyless news-feed default) and falls back to the news feed by itself if a keyed engine fails — read its `FALLBACK:` line to know which engine answered. If your host wired the freshness gate, WebSearch/WebFetch stay denied until this has run in the last 30 minutes — do not fight it, run the ladder.
...
4. If the ladder exits 2 (EVERY engine blocked, including the news feed — rare), the stamp opens the gate with a warning: you may WebSearch/WebFetch, but label EVERY changeable-fact claim `undated/unverified — ladder blocked`, prefer official primary pages, and never fall back to "what I remember".
